...
2. LEVEL 1 — Run automated scan:
   $PYTHON ~/.kilocode/skills/aif-skill-generator/scripts/security-scan.py <skill-path>
3. Check exit code:
...
2. **LEVEL 1** — Run automated scanner:
   ```bash
   $PYTHON ~/.kilocode/skills/aif-skill-generator/scripts/security-scan.py <path>
   ```
...
```bash
npx skills install --agent kilo <name>
$PYTHON ~/.kilocode/skills/aif-skill-generator/scripts/security-scan.py <installed-path>
```
...
```bash
$PYTHON ~/.kilocode/skills/aif-skill-generator/scripts/security-scan.py ./skill-name/
```
...
```bash
python ~/.kilocode/skills/dependency-graph/scripts/visualize.py $ARGUMENTS
```
```
...
1. **Local**: Keep in `~/.kilocode/skills/` for personal use
2. **Project**: Add to `.kilocode/skills/` and commit
